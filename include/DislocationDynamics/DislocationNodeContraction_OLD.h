            ...
            if (!(DN.simulationParameters.isPeriodicSimulation() && (nA->isBoundaryNode() || nB->isBoundaryNode()))) //Added for avoiding node contraction with boundary node
            {
                ...
                { // nA and nB cannot be moved to each other. The calculation of a third point is necessary
                    ...
                    { // neither nA nor nB are on bounding box
                        ...
                        { // both nodes confined by at most one plane
                                    ...
                                    switch (temp.size())
                                    {
                                        case 2:
                                        {
                                            ...
                                            if (DN.simulationParameters.isPeriodicSimulation())
                                            {
                                                ...
                                                {
                                                    VerboseNodeContraction(1, "DislocationNodeContraction case 8b2a" << std::endl;);
                                                    return contractToPosition(nA, nB, cutLine.snap(0.5 * (nA->get_P() + nB->get_P())), maxRange);
                                                }
                                                else
                                                {
                                                    VerboseNodeContraction(1, "DislocationNodeContraction case 8b2b" << std::endl;);
                                                    return false;
                                                }
                                            }
                                            else
                                            {
                                                VerboseNodeContraction(1, "DislocationNodeContraction case 8b2c" << std::endl;);
                                                return contractToPosition(nA, nB, cutLine.snap(0.5 * (nA->get_P() + nB->get_P())), maxRange);
                                            }
                                        ...
                                        case 1:
                                        {
                                            ...
                                            if (DN.simulationParameters.isPeriodicSimulation())
                                            {
                                                ...
                                                if ((cutLine.snap(0.5 * (nA->get_P() + nB->get_P())) - cutLine.snapToInfiniteLine(0.5 * (nA->get_P() + nB->get_P()))).squaredNorm() < FLT_EPSILON)
                                                {
                                                    VerboseNodeContraction(1, "DislocationNodeContraction case 8b1a" << std::endl;);
                                                    return contractToPosition(nA, nB, cutLine.snap(0.5 * (nA->get_P() + nB->get_P())), maxRange);
                                                }
                                                else
                                                {
                                                    VerboseNodeContraction(1, "DislocationNodeContraction case 8b1b" << std::endl;);
                                                    return false;
                                                }
                                            }
                                            else
                                            {
                                                VerboseNodeContraction(1, "DislocationNodeContraction case 8b1c" << std::endl;);
                                                return contractToPosition(nA, nB, cutLine.snap(0.5 * (nA->get_P() + nB->get_P())), maxRange);
                                            }
                                            break;
                                        }
